...
import (
	...
	keep "github.com/Bococoin/core/x/staking/keeper"
	"github.com/Bococoin/core/x/staking/types"
)

const WarnDelegationDisabled = "WARNING!!! Delegations is disabled by network parameter"

func TestValidatorByPowerIndex(t *testing.T) {
	validatorAddr, validatorAddr3 := sdk.ValAddress(keep.Addrs[0]), sdk.ValAddress(keep.Addrs[1])

	initPower := int64(5000000)
...
func TestDuplicatesMsgCreateValidator(t *testing.T) {
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, boco.DefaultMinValidatorSelfDelegation*10)

	addr1, addr2 := sdk.ValAddress(keep.Addrs[0]), sdk.ValAddress(keep.Addrs[1])
	pk1, pk2 := keep.PKs[0], keep.PKs[1]

	valTokens := sdk.TokensFromConsensusPower(boco.DefaultMinValidatorSelfDelegation)
...
func TestInvalidPubKeyTypeMsgCreateValidator(t *testing.T) {
	...
	msgCreateValidator := NewTestMsgCreateValidator(addr, invalidPk, sdk.NewInt(boco.DefaultMinValidatorSelfDelegation))
	res, err := handleMsgCreateValidator(ctx, msgCreateValidator, keeper)
	require.Error(t, err)
	require.Nil(t, res)
...
func TestLegacyValidatorDelegations(t *testing.T) {
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, int64(boco.DefaultMinValidatorSelfDelegation*10))
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		unbondAmt := sdk.NewCoin(boco.DefaultDenom, bondAmount)
		...
		t.Log(WarnDelegationDisabled)
	}
}

func TestIncrementsMsgDelegate(t *testing.T) {
	initPower := int64(boco.DefaultMinValidatorSelfDelegation * 10)
	initBond := sdk.TokensFromConsensusPower(boco.DefaultMinValidatorSelfDelegation)
...
func TestIncrementsMsgUnbond(t *testing.T) {
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		unbondAmt := sdk.NewCoin(boco.DefaultDenom, sdk.NewInt(10))
		...
		for _, c := range errorCases {
			unbondAmt := sdk.NewCoin(boco.DefaultDenom, c)
			msgUndelegate := NewMsgUndelegate(delegatorAddr, validatorAddr, unbondAmt)
			res, err = handleMsgUndelegate(ctx, msgUndelegate, keeper)
			require.Error(t, err)
			require.Nil(t, res)
		...
		unbondAmt = sdk.NewCoin(boco.DefaultDenom, leftBonded)
		msgUndelegate = NewMsgUndelegate(delegatorAddr, validatorAddr, unbondAmt)
		res, err = handleMsgUndelegate(ctx, msgUndelegate, keeper)
...
func TestMultipleMsgCreateValidator(t *testing.T) {
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		for i, validatorAddr := range validatorAddrs {
			...
			unbondAmt := sdk.NewCoin(boco.DefaultDenom, sdk.TokensFromConsensusPower(10))
...
func TestMultipleMsgDelegate(t *testing.T) {
	...
	msgCreateValidator := NewTestMsgCreateValidator(validatorAddr, keep.PKs[0], sdk.NewInt(boco.DefaultMinValidatorSelfDelegation))
	res, err := handleMsgCreateValidator(ctx, msgCreateValidator, keeper)
	require.NoError(t, err)
	require.NotNil(t, res)
...
func TestValidatorQueue(t *testing.T) {
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		unbondAmt := sdk.NewCoin(boco.DefaultDenom, delTokens)
		msgUndelegateValidator := NewMsgUndelegate(sdk.AccAddress(validatorAddr), validatorAddr, unbondAmt)
		res, err = handleMsgUndelegate(ctx, msgUndelegateValidator, keeper)
		require.NoError(t, err)
		require.NotNil(t, res)
...
func TestRedelegationPeriod(t *testing.T) {
	ctx, AccMapper, keeper, _ := keep.CreateTestInput(t, false, boco.DefaultMinValidatorSelfDelegation)
	...
	require.Equal(t, amt1.Sub(sdk.NewInt(boco.DefaultMinValidatorSelfDelegation)).Int64(), amt2.Int64(), "expected coins to be subtracted")

	msgCreateValidator = NewTestMsgCreateValidator(validatorAddr2, keep.PKs[1], sdk.NewInt(boco.DefaultMinValidatorSelfDelegation))
	res, err = handleMsgCreateValidator(ctx, msgCreateValidator, keeper)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	if keeper.IsDelegateEnabled(ctx) {
		// begin redelegate
		redAmt := sdk.NewCoin(boco.DefaultDenom, sdk.NewInt(10))
...
func TestTransitiveRedelegation(t *testing.T) {
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, boco.DefaultMinValidatorSelfDelegation)
	validatorAddr := sdk.ValAddress(keep.Addrs[0])
	...
	msgCreateValidator = NewTestMsgCreateValidator(validatorAddr3, keep.PKs[2], sdk.NewInt(boco.DefaultMinValidatorSelfDelegation))
	res, err = handleMsgCreateValidator(ctx, msgCreateValidator, keeper)
	require.NoError(t, err)
	require.NotNil(t, res)
...
func TestMultipleRedelegationAtSameTime(t *testing.T) {
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		redAmt := sdk.NewCoin(boco.DefaultDenom, valTokens.QuoRaw(2))
...
func TestMultipleUnbondingDelegationAtSameTime(t *testing.T) {
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		selfDelAddr := sdk.AccAddress(valAddr) // (the validator is it's own delegator)
		unbondAmt := sdk.NewCoin(boco.DefaultDenom, valTokens.QuoRaw(2))
...
func TestUnbondingWhenExcessValidators(t *testing.T) {
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, boco.DefaultMinValidatorSelfDelegation*5)
	...
	valTokens1 := sdk.TokensFromConsensusPower(boco.DefaultMinValidatorSelfDelegation + 500*boco.OneCoin)
	...
	valTokens2 := sdk.TokensFromConsensusPower(boco.DefaultMinValidatorSelfDelegation + 300*boco.OneCoin)
	...
	valTokens3 := sdk.TokensFromConsensusPower(boco.DefaultMinValidatorSelfDelegation)
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		unbondAmt := sdk.NewCoin(boco.DefaultDenom, valTokens2)
...
func TestBondUnbondRedelegateSlashTwice(t *testing.T) {
	...
	if keeper.IsDelegateEnabled(ctx) {
		...
		unbondAmt := sdk.NewCoin(boco.DefaultDenom, sdk.TokensFromConsensusPower(4))
		...
		redAmt := sdk.NewCoin(boco.DefaultDenom, sdk.TokensFromConsensusPower(6))
...
func TestInvalidCoinDenom(t *testing.T) {
	...
	validCoin := sdk.NewCoin(boco.DefaultDenom, valTokens)
	oneCoin := sdk.NewCoin(boco.DefaultDenom, sdk.OneInt())
