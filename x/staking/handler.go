...
import (
	...
	"github.com/Bococoin/core/x/staking/keeper"
...
func handleMsgCreateValidator(ctx sdk.Context, msg types.MsgCreateValidator, k keeper.Keeper) (*sdk.Result, error) {
	...
	validator.MinSelfDelegation = sdk.NewInt(k.MinSelfDelegation(ctx))

	// Amount mast be greater then MinSelfDelegation parameter
	if msg.Value.Amount.LT(validator.MinSelfDelegation) {
		return nil, sdkerrors.Wrapf(
			ErrSelfDelegationBelowMinimum,
			"got: %d, valid: %d", msg.Value.Amount.Int64(), k.MinSelfDelegation(ctx),
		)
	}
...
func handleMsgDelegate(ctx sdk.Context, msg types.MsgDelegate, k keeper.Keeper) (*sdk.Result, error) {
	// Check is delegation enabled
	if !k.IsDelegateEnabled(ctx) {
