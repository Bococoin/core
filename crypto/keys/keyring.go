...
import (
	...
	"github.com/Bococoin/core/client/input"
	"github.com/Bococoin/core/crypto/keys/keyerror"
	"github.com/Bococoin/core/crypto/keys/mintkey"
...
func NewKeyring(
	appName, backend, rootDir string, userInput io.Reader, pass string, opts ...KeybaseOption,
) (Keybase, error) {
	...
	case BackendCmdPass:
		db, err = keyring.Open(newFileBackendKeyringCmdPassConfig(appName, rootDir, pass))
	default:
...
func newFileBackendKeyringCmdPassConfig(name, dir string, pass string) keyring.Config {
	fileDir := filepath.Join(dir, fmt.Sprintf(keyringDirNameFmt, name))
	return keyring.Config{
		AllowedBackends:  []keyring.BackendType{keyring.FileBackend},
		ServiceName:      name,
		FileDir:          fileDir,
		FilePasswordFunc: fixedStringPrompt(pass),
	}
}
