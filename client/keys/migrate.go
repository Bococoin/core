...
func runMigrateCmd(cmd *cobra.Command, args []string) error {
	...
	if viper.GetBool(flags.FlagDryRun) {
		...
		keybase, err = keys.NewKeyring(keyringServiceName, "test", tmpDir, buf, viper.GetString(flagPass))
	} else {
		keybase, err = keys.NewKeyring(keyringServiceName, viper.GetString(flags.FlagKeyringBackend), rootDir, buf, viper.GetString(flagPass))
	}
	if err != nil {
