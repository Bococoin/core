...
func NewCLIContextWithInputAndFrom(input io.Reader, from string) CLIContext {
	...
	if err != nil {
		if viper.GetString(flags.FlagPass) != "" {
			fmt.Printf( "wrong integration passphrase")
		} else {
...
func GetFromFields(input io.Reader, from string, genOnly bool) (sdk.AccAddress, string, error) {
	...
	keybase, err := keys.NewKeyring(sdk.KeyringServiceName(),
		viper.GetString(flags.FlagKeyringBackend), viper.GetString(flags.FlagHome), input, viper.GetString(flags.FlagPass))
	if err != nil {
		return nil, "", err
	}
